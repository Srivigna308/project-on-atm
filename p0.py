...
class BankUi:
    ...
    def family_access_auth(self):
        ...
        # Submit button (calls verify_user, which handles family access logic)
        self.submit_btn = Button(
            ...
            fg="white",
            font=ARIAL,
            command=self.verify_user  # <-- Correct method
        )
    ...
    def verify_user(self):
        ...
        if self.selected_verification == "Family Access":
            data = pd.read_csv('bank_details.csv')
            # Force both to string type for matching
            data['account_number'] = data['account_number'].astype(str)
            real_user_str = str(self.real_user)

            user_data = data[data['account_number'] == real_user_str]
            ...
            if 'family_name' not in data.columns or 'shared_pin' not in data.columns:
                messagebox.showerror("Authentication Failed",
                                     "Missing necessary data (family name or shared pin) in database!")
                return

            # Step 2: Filter user
            user_data =data[data.loc[:,'account_number'] == self.real_user]
            print("[DEBUG] Filtered User Data:\n", user_data)
            if user_data.empty:
                messagebox.showerror("Authentication Failed", "Account not found!")
                return

            # Step 3: Match credentials'shared_pin'
            expected_family_name = user_data['family_name'].values[0]
            expected_pin = user_data['shared_pin'].values[0]

            if family_name.strip() == expected_family_name and shared_pin.strip() == expected_pin:
                messagebox.showinfo("Login Successful", "Family Access granted!")
                self.final_page()
            else:
                messagebox.showerror("Authentication Failed", "Invalid family name or shared PIN!")
            return
    ...
    def user_account_transfer_transc(self):
        ...
        to_acc = str(to_acc)
        amount = float(amount)  # Now safe to convert
        ...
        # Sender balance
        sender_info = data[data['account_number'] == str(self.real_user)]

        if sender_info.empty:
            messagebox.showinfo("Transfer Info", "Sender account not found.")
            return

        sender_balance = float(sender_info['account_balance'].values[0])
    ...
    def user_balance(self):
        ...
        try:
            data = pd.read_csv('bank_details.csv', on_bad_lines='skip')
            account_info = data[data['account_number'].astype(str) == str(self.real_user)]
            print("Loaded CSV successfully")
            print(f"Found rows: {len(account_info)}")
    ...
    def user_deposit_trans(self):
        ...
        try:
            ...
            # FIX: convert account_number to string
            data['account_number'] = data['account_number'].astype(str)

            update_data = data.set_index('account_number')

            if str(self.real_user) not in update_data.index:
    ...
    def _complete_deposit(self, popup, update_data, deposit_amount):
        ...
        # Update balance
        update_data.loc[str(self.real_user), 'account_balance'] = (
                float(update_data.loc[str(self.real_user), 'account_balance']) + deposit_amount
        )
    ...
    def _complete_withdrawal(self, popup, update_data, withdraw_amount):
        try:
            ...
            current_balance = float(update_data.loc[str(self.real_user), 'account_balance'])

            if withdraw_amount > current_balance:
                messagebox.showerror("Insufficient Balance", "You do not have enough balance to withdraw.")
                self.user_balance()
                return
            ...
            self.user_balance()  # Go to updated balance page
    ...
    def video_check(self):
        ...
        try:
            ...
            if Counter(real_user_list).most_common(1)[0][0] == 'unknown':
                ...
                # DON'T overwrite real_user here. Only proceed to password verification.
